...
while m != len(arr):
    ...
    if j + 1 != len(arr):
        ...
        for p in range(1, len(str_image)):
            dict_image[str_image[p].split('=')[0]] = str_image[p].split('=')[1]
    ...
    if arr_split[i] == '#':
        ...
        if not image_h_arr:
            ...
            while temp_3 == True:
                for q in range(len(interval)):
                    if curr[1] <= interval[q][1] or q == len(interval) - 1:
                        if curr[1] == interval[q][0] and curr[1] + len(arr_split[i])*c <= interval[q][1]:
                            curr[1] += len(arr_split[i])*c
                            temp_3 = False
                            break
                        elif curr[1] < interval[q][1] and curr[1] >= interval[q][0] and curr[1] + len(arr_split[i])*c + c <= interval[q][1]:
                            curr[1] += len(arr_split[i])*c + c
                            temp_3 = False
                            break
                        else:
                            if q + 1 != len(interval) and interval[q + 1][0] > curr[1]:
                                ...
                                for u in range(len(coords_arr)):
                                    ...
                                    if image_h_arr[u] <= 0:
                                        ...
                                        for b in range(len(interval) - 1):
                                            if coords_arr[u][0] == interval[b][1]:
                                                interval[b] = [interval[b][0], interval[b+1][1]]
                                                interval.pop(b+1)
                                                temp_4 += 1
                                                break
                                        coords_arr.pop(u)
        ...
        if floating[1] == 0:
            if curr[1] + arr_split[i].dx < 0:
                ...
                ans.append([w - arr_split[i].width, curr[0] + arr_split[i].dy])
            else:
                ans.append([curr[1] + arr_split[i].dx, curr[0] + arr_split[i].dy])
                floating[0] = arr_split[i].dy
                floating[1] = arr_split[i].dx + arr_split[i].width
        else:
            if floating[1] + arr_split[i].width <= w:
                if curr[1] + floating[1] + arr_split[i].dx < 0:
                    ans.append([0 + floating[1], curr[0] + floating[0] + arr_split[i].dy])
                elif curr[1] + floating[1] + arr_split[i].dx + arr_split[i].width >= w:
                    ans.append([w - arr_split[i].width, curr[0] + floating[0] + arr_split[i].dy])
                else:
                    ans.append([curr[1] + floating[1] + arr_split[i].dx, curr[0] + floating[0] +  arr_split[i].dy])
                floating[0] += arr_split[i].dy
                floating[1] += arr_split[i].dx + arr_split[i].width
            else:
                floating[0] += arr_split[i].dy
        ...
        if arr_split[i].layout == 'embedded':
            if not image_h_arr:
                ...
                while temp_3 == True:
                    for q in range(len(interval)):
                        if curr[1] == interval[q][0] and curr[1] + arr_split[i].width <= interval[q][1]:
                            ans.append([curr[1], curr[0]])
                            curr_h = max(arr_split[i].height, curr_h, h)
                            curr[1] += arr_split[i].width
                            temp_3 = False
                            break
                        elif curr[1] < interval[q][1] and curr[1] >= interval[q][0] and curr[1] + arr_split[i].width + c <= interval[q][1]:
                            ans.append([curr[1] + c, curr[0]])
                            curr_h = max(arr_split[i].height, curr_h, h)
                            curr[1] += arr_split[i].width + c
                            temp_3 = False
            ...
            if not image_h_arr:
                if arr_split[i].width + curr[1] > w:
                    ...
                    coords_arr.append([curr[1], curr[1] + arr_split[i].width])
                    interval[0] = [0, curr[1]]
                ...
                while temp_3 == True:
                    for q in range(len(interval)):
                        if curr[1] <= interval[q][1] or q == len(interval) - 1:
                            if curr[1] < interval[q][1] and curr[1] >= interval[q][0] and curr[1] + arr_split[i].width <= interval[q][1]:
                                ans.append([curr[1], curr[0]])
                                image_h_arr.append(arr_split[i].height)
                                coords_arr.append([curr[1], curr[1] + arr_split[i].width])
                                interval.insert(q+1, [curr[1] + arr_split[i].width, interval[q][1]])
